...
complex<double> evalSphHarm(int l, int m, Vector &v) {
	double r = v.len();
	...
	double legendreVal = PLM(l, m, cos(theta));
	double coef = pow(-1., m) * sqrt((2 * l + 1) * fact(l - m) / (4 * PI * fact(l + m))) * legendreVal;
	return complex<double>(coef * cos(m * phi), coef * sin(m * phi));
}


vector<complex<double> > calcQlms(Atom *a, int l) {
	...
	for (int m=-l; m<=l; m++) {
		...
		for (int i=0; i<numNeighbors; i++) {
			...
			sphHarmSum += evalSphHarm(l, m, vToN);
		}
		if (numNeighbors) {
...
complex<double> calcQlmBar(Atom *a, int l, int m) {
...
void calcQlAvg(vector<Atom *> &atoms, int l) {
	double PI = 3.141592654;
